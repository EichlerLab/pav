...
def call_from_align(caller_resources, min_anchor_score=pavlib.const.DEFAULT_MIN_ANCHOR_SCORE, verbose=False, dot_basename=None):
    ...
    :param dot_basename: Name of dot graph file to write if not None. File is gzipped if ends with the filename ends
        with '.gz'.
    ...
    for query_id in caller_resources.df_align_qry['QRY_ID'].unique():
        ...
        if dot_basename is not None:
            dot_filename = dot_basename + f'{query_id}.dot.gz'
